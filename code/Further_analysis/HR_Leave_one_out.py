# -*- coding: utf-8 -*-
"""
Created on Sat Nov 13 21:08:36 2021

@author: harisushehu
"""


#Go to the project directory/path and run  /home/harisushehu/myenv/bin/python HR_Leave_one_out.py
...
csvFileName = '../data/results_HR_LeaveOneOut.csv'
...
for start in range(5, 154):
    ...
    keras_callbacks   = [
          ...
          ModelCheckpoint("best_HR_LeaveOneOut_model.hdf5", monitor='val_loss', save_best_only=True, mode='min', save_freq='epoch')]
    ...
    history = regressor.fit(X_train, y_train, epochs = 100, verbose = 1, callbacks=keras_callbacks, batch_size = 64, validation_data = (X_val, y_val))
    
    from tensorflow.keras.models import load_model
    # load best model from single file
    regressor = load_model('best_HR_LeaveOneOut_model.hdf5')
    ...
    row_contents = [str(start), str(rmse), str(nrmse_val), str(Linear_rmse), str(NLinear_nrmse), str(DT_rmse), str(NDT_nrmse)]
    ...
    plt.ylabel('heartrate_mean', fontsize=10)
    ...
    filename = "../data/HR_LeaveOneOut/Pred_" + str(start)
