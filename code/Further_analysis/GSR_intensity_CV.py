# -*- coding: utf-8 -*-
"""
Created on Sun Oct 24 23:16:56 2021
...
#Go to the project directory/path and run  /home/harisushehu/myenv/bin/python GSR_intensity_CV.py
...
csvFileName = '../data/results_GSR_intensity_CV.csv'
...
for normloop in range(1, 154): #154
    ...
    if flag == True:
        ...
        video_list = [146, 140, 1643, 1362, 962, 1318, 2131, 404, 98, 1774, 153, 1010, 778, 436, 24, 1612, 725, 483, 1761, 1269, 593, 1744, 81, 1590]
        ...
        for j in range(0, len(video_list)):
             ...
             path = ("/EMAP_datasetPath/Features/") + ("Features_P" + participant_no + "-T") + partNo + (".csv")
             filename = path
             ...
             if (val % 24) == 3:
                 ...
                 sixth_part.extend(full_list)
                 full_list = []
                 
                 
             elif (val % 24) == 21:
                 
                 seventh_part.extend(full_list)
                 full_list = []
                 
             elif (val % 24) == 0:
                 
                 eighth_part.extend(full_list)
                 full_list = []
             
           
list_of_lists = [first_part, second_part, third_part, fourth_part, fifth_part, sixth_part, seventh_part, eighth_part]
...
for i in range(len(list_of_lists)):
    ...
    keras_callbacks   = [
          ...
          ModelCheckpoint("best_GSR_stratified_CVmodel.hdf5", monitor='val_loss', save_best_only=True, mode='min', save_freq='epoch')]
    ...
    history = regressor.fit(X_train, y_train, epochs = 100, verbose = 1, callbacks=keras_callbacks, batch_size = 128, validation_split = 0.3)

    
    from tensorflow.keras.models import load_model
    # load best model from single file
    regressor = load_model('best_GSR_stratified_CVmodel.hdf5')
    ...
    filename = "../data/GSR_intensity/Pred_" + str(i)
